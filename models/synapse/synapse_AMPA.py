...
# Datetime  : 2024/5/8
# User      : WuY
# File      : synapse_AMPA.py
# 这个文件收集 AMPA(一种兴奋性突触结构) 的各种写法
...
class synbase_AMPA1(Synapse):
    ...
    def __init__(self, pre, post, conn=None, synType="chem_exp", method="euler"):
        ...
        if self.synType == "chem_exp":
            self.syn = self.syn_chem_AMPA1  # 化学突触
        self._params()
        self._vars()
    ...
    def _dg_dt(self):
        # 只有一个变量的时候，返回必须加“ , ”
        dg_dt = -self.g / self.tau_AMPA
        return dg_dt,

    def syn_chem_AMPA1(self, pre_fire_launch=None, post_mem=None):
        """
        args:
